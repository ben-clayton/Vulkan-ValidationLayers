...
GlobalImageLayoutRangeMap *GetLayoutRangeMap(GlobalImageLayoutMap *map, const IMAGE_STATE &image_state) {
    assert(map);
    GlobalImageLayoutRangeMap *layout_map = nullptr;
    // This approach allows for a single hash lookup or/create new
    auto inserted = map->emplace(std::make_pair(image_state.image, layout_map));
    if (inserted.second) {
        assert(nullptr == inserted.first->second.get());
        layout_map = new GlobalImageLayoutRangeMap(image_state.range_encoder.SubresourceCount());
        inserted.first->second.reset(layout_map);
    } else {
        assert(nullptr != inserted.first->second.get());
        layout_map = inserted.first->second.get();
    }
    return layout_map;
}
